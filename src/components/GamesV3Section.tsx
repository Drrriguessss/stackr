...
import { optimalGamingAPIV3, type OptimalGamingV3Result } from '@/services/optimalGamingAPIV3'
...
export default function GamesV3Section({
  ...
  const [searchResults, setSearchResults] = useState<OptimalGamingV3Result[]>([])
  const [isSearching, setIsSearching] = useState(false)
  const [trendingGames, setTrendingGames] = useState<OptimalGamingV3Result[]>([])
  ...
  const loadTrendingGames = async () => {
    ...
    try {
      const trending = await optimalGamingAPIV3.getTrending(8)
      setTrendingGames(trending)
      setApiCalls(prev => prev + 1) // Track API calls
      console.log('🎮 [V3] Loaded trending with 1 API call')
    } catch (error) {
      console.error('🎮 [V3] Failed to load trending:', error)
    } finally {
  ...
  const handleSearch = async (query: string) => {
    ...
    setIsSearching(true)
    try {
      const results = await optimalGamingAPIV3.search(query, { limit: 12 })
      setSearchResults(results)
      setApiCalls(prev => prev + 2) // Track API calls (hybrid = 2 calls)
      console.log('🎮 [V3] Hybrid search completed with 2 API calls (recent + classic)')
    } catch (error) {
  ...
  const renderGameCard = (game: OptimalGamingV3Result) => (
    <div
      key={game.id}
  ...
  return (
    <div className="w-full">
      {/* Header with Purple Gradient for V3 */}
      <div 
        ...
        <div className="relative z-10">
          <div className="flex items-center justify-between mb-4">
            <div className="flex items-center gap-3">
              <Gamepad2 size={32} />
              <div>
                <h2 className="text-2xl font-bold">Games V3</h2>
                <p className="text-purple-100 text-sm">HYBRID STRATEGY: Recent + Classic search fusion</p>
              </div>
            </div>
          ...
          <div className="relative">
            ...
            <input
              ...
              placeholder="Search games with V3 (hybrid strategy)..."
      ...
      <div className="bg-gray-50 p-6 rounded-b-xl">
        ...
        <div className="mt-8 p-4 bg-purple-50 rounded-lg">
          ...
          <ul className="mt-2 text-xs text-purple-600 space-y-1">
            <li>• Hybrid strategy: 2 RAWG API requests (recent + classic)</li>
            <li>• Recent games (2023-2025+) searched with date filter</li>
            <li>• Classic games searched with relevance ranking</li>
            <li>• Results intelligently merged and ranked</li>
            <li>• Results cached for 15 minutes</li>
            <li>• Total API calls this session: {apiCalls}</li>
            <li>• 🎯 OPTIMAL: Franchise games sorted by date, others by relevance</li>
          </ul>
        </div>
      </div>
    </div>
