...
interface SearchModalProps {
  isOpen: boolean
  onClose: () => void
  onAddToLibrary: (item: SearchResult, status: string) => void
}
...
export default function SearchModal({ isOpen, onClose, onAddToLibrary }: SearchModalProps) {
  ...
  const performSearch = async (searchQuery: string) => {
    ...
    try {
      // Recherche parallèle dans toutes les APIs
  ...
  const searchGames = async (query: string): Promise<SearchResult[]> => {
    try {
      ...
      return data.results?.map((game: any) => ({
        ...
        year: new Date(game.released).getFullYear() || 2024,
  ...
  const searchMovies = async (query: string): Promise<SearchResult[]> => {
    try {
      ...
      return data.Search?.slice(0, 5).map((movie: any) => ({
        ...
        title: movie.Title,
        director: 'Unknown', // OMDb search ne donne pas le réalisateur
        year: parseInt(movie.Year) || 2024,
        rating: 0, // Pas de rating dans la recherche OMDb
  ...
  const searchMusic = async (query: string): Promise<SearchResult[]> => {
    try {
      ...
      return data.results?.map((album: any) => ({
        ...
        year: new Date(album.releaseDate).getFullYear() || 2024,
        rating: 0,
  ...
  const searchBooks = async (query: string): Promise<SearchResult[]> => {
    try {
      ...
      return data.items?.map((book: any) => ({
        ...
        year: parseInt(book.volumeInfo.publishedDate) || 2024,
            ...
            <div className="space-y-2 p-4">
              {results.map((result) => (
                <div
                  ...
                  onClick={() => {
                    // Ici on peut ouvrir une modal de détails ou ajouter directement
                ...
                >
                  ...
                  <button
                    onClick={(e) => {
                      e.stopPropagation()
                      onAddToLibrary(result, 'want-to-watch') // Status par défaut
                      onClose()
                    }}
