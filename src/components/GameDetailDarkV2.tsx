...
import { ArrowLeft, Star, Send, ChevronDown, ChevronRight, Play, Share, Calendar, DollarSign, Tag, Users, Check, FileText, Image, Video, Info, MessageCircle } from 'lucide-react'
...
export default function GameDetailDarkV2({ 
  ...
  const handleGameSheetSave = () => {
    console.log('Game sheet data saved:', gameSheetData)
    setShowGameSheet(false)
  }
                ...
                <div className="flex gap-4 items-start mb-4">
                  ...
                  <div className="flex-1 pt-1">
                    ...
                    <div className="flex flex-wrap items-center gap-2 text-xs text-gray-500">
                      ...
                      {gameDetail.platforms?.[0] && (
                        <>
                          <span className="text-gray-600">•</span>
                          <span>{gameDetail.platforms.slice(0, 2).map(p => p.platform.name).join(', ')}</span>
                        </>
                      )}
                    </div>
                  </div>
          ...
          <div className="px-6 py-4 relative z-1">
            ...
            {activeTab === 'trailers' && (
              <div className="space-y-4">
                ...
                {/* Trailers */}
                {validatedTrailers.length > 0 && (
                  <div className="mb-6">
                    <h4 className="text-md font-medium text-white mb-3">Trailers</h4>
                    <div className="grid grid-cols-1 gap-4">
                      {validatedTrailers.slice(0, 3).map((trailer, index) => (
                        <div key={index} className="bg-gray-800 rounded-lg overflow-hidden">
                          <div className="aspect-video relative">
                            <iframe
                              ...
                              allowFullScreen
                              title={trailer.name || `Trailer ${index + 1}`}
                            />
                          </div>
                          <div className="p-3">
                            <h5 className="text-white text-sm font-medium">{trailer.name || `Trailer ${index + 1}`}</h5>
                          </div>
                        </div>
                      ))}
                    </div>
                ...
                {validatedTrailers.length === 0 && screenshots.length === 0 && (
                  <div className="text-gray-400 text-sm bg-gray-800/50 rounded-lg p-4">
        ...
        <div className="bg-gray-800 rounded-xl p-6 max-w-2xl w-full max-h-[80vh] overflow-y-auto">
          ...
          <div className="space-y-4">
            ...
            <div>
              ...
              <input
                type="text"
                value={gameSheetData.customTags}
                onChange={(e) => setGameSheetData({...gameSheetData, customTags: e.target.value})}
                placeholder="e.g., multiplayer, story-rich"
                className="w-full px-3 py-2 bg-gray-700 text-white rounded-lg focus:outline-none focus:ring-2 focus:ring-purple-500"
              />
            </div>
          </div>
